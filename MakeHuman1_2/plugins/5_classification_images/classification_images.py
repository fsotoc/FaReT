...
def generate_CI_info(base_model, base_expression, trials, shape_parameters, pose_parameters, save_path, SD=.3, rot_X=0, rot_Y=0, material_file="young_caucasian_avg", render_function=None):
    ...
    vertex_deviations = DataFrame()

    vertex_deviations.append(OrderedDict((("anti", -1),("trial", -1))+tuple(zip(vertex_idx, base_vertices))))
    ...
    for trial in range(trials):
        ...
        vertex_deviations.append(OrderedDict((("anti", False), ("trial", trial))+tuple(zip(vertex_idx, human.mesh.r_coord[vertex_idx]-base_vertices))))
        ...
        vertex_deviations.append(OrderedDict((("anti", True), ("trial", trial))+tuple(zip(vertex_idx, human.mesh.r_coord[vertex_idx]-base_vertices))))
    ...
    vertex_deviations.to_csv(os.path.join(save_path, "CI_vertex_deviations.csv"))
